...
EventDispatcher.prototype.MonkeyDispatchEvent = function(event,fn){
	var self = this;
...
function MonkeyEvent(type,data){
    this.type=type;
    this.data=data;
    this.target=null;
}
...
var MK_Loading = (function(){
	...
	MK_Loading.prototype.upDate=function(val){
		...
		if(_s.loadUp)
			_s.loadUp(val);		
		_s.MonkeyDispatchEvent(new MonkeyEvent("update",val));
	};
			...
			__up = function(){
				...
				if(LDprogress <= 100){
					...
					if(speed == 100){
						...
						_s.MonkeyDispatchEvent(new MonkeyEvent("complete",speed));
					}
				};
			};
...
var MK_animateImages=(function(){
	...
	animateImages.prototype.play=function(order){//播放
		var self = this;
		order = order || 1;
		...
		self.ITV = setInterval(function(){
			if((self.progress >= self.endIndex && order == 1) || (self.progress < self.startIndex && order == -1)){
				
				if(self.loop)
					self.progress = order == 1 ? self.startIndex : self.endIndex - 1;
				else{
					self.progress = order == -1 ? self.startIndex : self.endIndex - 1;
					self.stop();
				}

				self.MonkeyDispatchEvent(new MonkeyEvent("complete",self.progress));

				return;
			}
			self.toProgress(self.progress);
			self.MonkeyDispatchEvent(new MonkeyEvent("update",self.progress));
			self.MonkeyDispatchEvent(new MonkeyEvent(self.progress,self.progress));

			self.progress += self.step * order ;
			
		}, self.times);

	};

	animateImages.prototype.inverse = function(){
		this.play(-1);
	}
