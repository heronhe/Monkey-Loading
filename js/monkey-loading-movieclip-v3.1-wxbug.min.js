...
break;case"ajax":f.load=function(){f.count++;f.ajaxFn();};}})(c);}};a.prototype.upDate=function(d){var c=this;if(c.loadUp){c.loadUp(d);}c.MonkeyDispatchEvent(new MonkeyEvent("update",d));
...
if(f<=100){if(e.p.LDtween&&d<f){d++;}else{d=f;}e.upDate(d);if(d==100){clearInterval(e.set);if(e.callback){e.callback();}e.MonkeyDispatchEvent(new MonkeyEvent("complete",d));
...
if(i.ITV||this.anmtList.length==0){return;}i.ITV=setInterval(function(){if(i.progress>=i.endIndex){i.MonkeyDispatchEvent(new MonkeyEvent("complete",i.progress));
if(i.loop){i.progress=i.startIndex;}else{i.stop();}return;}i.toProgress(i.progress);i.MonkeyDispatchEvent(new MonkeyEvent("update",i.progress));i.MonkeyDispatchEvent(new MonkeyEvent(i.progress,i.progress));
i.progress+=i.step;},i.times);};b.prototype.stopActivity=function(){var i=this;if(i.ITV){clearInterval(i.ITV);i.ITV=null;}};b.prototype.pause=function(){var i=this;
i.stopActivity();};b.prototype.stop=function(){var i=this;i.stopActivity();i.progress=i.startIndex;};b.prototype.creatElment=function(j){var i=this;return document.createElement(j);
