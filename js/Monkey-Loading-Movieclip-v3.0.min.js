...
}};EventDispatcher.prototype.dispatchEvent=function(f,e){var b=this;if(b.registorPool[f.type]){var a=b.registorPool[f.type];var d=false;for(var c=0,g=a.length;
...
}}}};function Event(a,b){this.type=a;this.data=b;this.target=null;}Function.prototype.getName=function(){return this.name||this.toString().match(/function\s*([^(]*)\(/)[1];
...
console.log(d.list);for(var c=0,e=d.list.length;c<e;c++){(function(g){var f=d.list[g];f.status=0;f.count=0;switch(f.type){case"img":f.e=f.e||new Image();
f.e.onload=function(){f.status=1;d.removeLD(f);};f.e.onerror=function(){if(f.count<d.errorCount){f.status=2;f.load();}else{f.status=3;d.removeLD(f);}};
break;case"ajax":f.load=function(){f.count++;f.ajaxFn();};}})(c);}};a.prototype.upDate=function(d){var c=this;if(c.loadUp){c.loadUp(d);}c.dispatchEvent(new Event("update",d));
};a.prototype.start=function(){var e=this;e.init();e.length=e.getLength();e.loads();var d=0,c=function(){var f=parseInt(e.suclist.length/e.length*100);
if(f<=100){if(e.p.LDtween&&d<f){d++;}else{d=f;}e.upDate(d);if(d==100){clearInterval(e.set);if(e.callback){e.callback();}e.dispatchEvent(new Event("complete",d));
}}};e.set=setInterval(c,e.timers);};a.prototype.add=function(c){var d=this;return typeof c==="object"?d.addLoadObject(c):[];};a.prototype.addLDstack=function(h){var g=this,c=g.isLoad(h.e),f;
...
i.startIndex=j;i.toProgress(j);};b.prototype.setEnd=function(j){var i=this;if(j>i.length){i.endIndex=i.length;}else{i.endIndex=j;}};b.prototype.play=function(){var i=this;
if(i.ITV||this.anmtList.length==0){return;}i.ITV=setInterval(function(){if(i.progress>=i.endIndex){i.dispatchEvent(new Event("complete",i.progress));if(i.loop){i.progress=i.startIndex;
}else{i.stop();}return;}i.toProgress(i.progress);i.dispatchEvent(new Event("update",i.progress));i.dispatchEvent(new Event(i.progress,i.progress));i.progress+=i.step;
},i.times);};b.prototype.stopActivity=function(){var i=this;if(i.ITV){clearInterval(i.ITV);i.ITV=null;}};b.prototype.pause=function(){var i=this;i.stopActivity();
};b.prototype.stop=function(){var i=this;i.stopActivity();i.progress=i.startIndex;};b.prototype.creatElment=function(j){var i=this;return document.createElement(j);
